...
#~~ Solution of Q4
  ...
  #Excluding the movie that tops the list in terms of Total Collections from bollywood dataset 
  #and Naming this dataset as "temp_bollywood"
  temp_bollywood <- subset(bollywood,Movie!=top_movie) #top_movie obtained in Q3
  
  #Now applying the which.max function on "temp_bollywood" dataset to get the desired output
  top_2_movie <- temp_bollywood$Movie[which.max(temp_bollywood$Tcollection)]
	
  #Removing "temp_bollywood" from the environment
	rm(temp_bollywood)
...
# Store the names of 4 movies in same sequence in movie_result vector
  ...
  movie_result <- bollywood$Movie[sapply(bollywood[,c("Ocollection","Wcollection","Fwcollection","Tcollection")],which.max)]
